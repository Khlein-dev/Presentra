...
const HandTracker = ({ onTrackingUpdate, onCleanup }) => {
	...
	const detectEyeContactAdvanced = (faceLandmarks) => {
		if (!faceLandmarks || faceLandmarks.length < 478)
			return { contact: false, gaze: "center" };
		...
		const eyeContactThreshold =
			FACIAL_REFERENCE_POINTS.eyes.eyeContactThreshold;
		const eyeCenterMin =
			FACIAL_REFERENCE_POINTS.eyes.eyeContactCenterX -
			eyeContactThreshold;
		const eyeCenterMax =
			FACIAL_REFERENCE_POINTS.eyes.eyeContactCenterX +
			eyeContactThreshold;
		...
		const hasEyeContact =
			avgIrisX > eyeCenterMin &&
			avgIrisX < eyeCenterMax &&
			Math.abs(leftIrisX - rightIrisX) <
				FACIAL_REFERENCE_POINTS.eyes.eyeSymmetry &&
			eyesOpen;
		...
		if (!hasEyeContact) {
			if (avgIrisX < FACIAL_REFERENCE_POINTS.eyes.eyeLookLeftThreshold) {
				...
				avgIrisX > FACIAL_REFERENCE_POINTS.eyes.eyeLookRightThreshold
			) {
				gaze = "right";
			} else {
				gaze = "away";
	...
	const detectMouthExpression = (faceLandmarks) => {
		...
		if (
			smoothedOpenness >= refs.smileMouthOpen.min &&
			smoothedOpenness <= refs.smileMouthOpen.max &&
			smoothedWidth >= refs.smileMouthWidth &&
			smoothedRise > refs.smileCornerRise
		) {
			console.log("✓ SMILE (smoothed):", {
				openness: smoothedOpenness.toFixed(3),
				width: smoothedWidth.toFixed(3),
				rise: smoothedRise.toFixed(3),
			});
			return "smile";
		}
		...
		if (
			smoothedOpenness >= refs.frownMouthOpen.min &&
			smoothedOpenness <= refs.frownMouthOpen.max &&
			smoothedRise < refs.frownCornerDrop
		) {
			console.log("✓ FROWN (smoothed):", {
				openness: smoothedOpenness.toFixed(3),
				drop: smoothedRise.toFixed(3),
			});
			return "frown";
		}

		console.log("Mouth metrics (smoothed):", {
			openness: smoothedOpenness.toFixed(3),
			width: smoothedWidth.toFixed(3),
			rise: smoothedRise.toFixed(3),
		});
	...
	const detectEyebrowExpression = (faceLandmarks) => {
		...
		const eyebrowDistance = Math.abs(
		...
		// MAD/ANGRY detection
		const leftSlanted =
			leftEyebrowAngle >= refs.madAngleRange.min &&
			leftEyebrowAngle <= refs.madAngleRange.max;
		const rightSlanted =
			rightEyebrowAngle >= refs.madAngleRange.min &&
			rightEyebrowAngle <= refs.madAngleRange.max;
		const eyebrowsClose = eyebrowDistance < refs.madDistance;

		if (leftSlanted && rightSlanted && eyebrowsClose) {
			console.log("✓ MAD:", {
				leftAngle: leftEyebrowAngle.toFixed(1),
				rightAngle: rightEyebrowAngle.toFixed(1),
				distance: eyebrowDistance.toFixed(3),
			});
			return "mad";
		}

		console.log("Eyebrow metrics:", {
			leftAngle: leftEyebrowAngle.toFixed(1),
			rightAngle: rightEyebrowAngle.toFixed(1),
			distance: eyebrowDistance.toFixed(3),
		});
	...
	const detectHandMovement = (handLandmarks, lastHandPos) => {
        ...
        if (thumb && pinky && thumb.visibility > 0.3 && pinky.visibility > 0.3) {
			...
			if (lastHandPos.fingerSpread === null) {
				...
				if (
					fingerSpread >=
					HAND_GESTURE_REFERENCES.openHand.fingerSpreadMin
				) {
					movement = "openHand";
					lastHandPos.fingerSpread = fingerSpread;
				}
				// Close Hand: fingers close together < 0.08 units apart
					...
					fingerSpread <=
					HAND_GESTURE_REFERENCES.closeHand.fingerSpreadMax
				) {
					movement = "closeHand";
					lastHandPos.fingerSpread = fingerSpread;
				}
		...
		if (
			indexFinger &&
			middleFinger &&
			indexFinger.visibility > 0.3 &&
			middleFinger.visibility > 0.3
		) {
	...
	const onUserMedia = async () => {
		try {
			...
			holistic.onResults((results) => {
				...
				const mouthExpression = detectMouthExpression(
					results.faceLandmarks,
				);
				const eyebrowExpression = detectEyebrowExpression(
					results.faceLandmarks,
				);
				...
				if (results.leftHandLandmarks) {
					leftMovement = detectHandMovement(
						results.leftHandLandmarks,
						handPosRef.current.leftHand,
					);
				}
				if (results.rightHandLandmarks) {
					rightMovement = detectHandMovement(
						results.rightHandLandmarks,
						handPosRef.current.rightHand,
					);
				}
				...
				if (results.leftHandLandmarks) {
					...
					if (leftThumb && leftPinky) {
						const leftFingerSpread = Math.abs(
							leftThumb.x - leftPinky.x,
						);
						...
						canvasCtx.fillText(
							`L-Spread: ${leftFingerSpread.toFixed(2)}`,
							10,
							20,
						);
				...
				if (results.rightHandLandmarks) {
					...
					if (rightThumb && rightPinky) {
						const rightFingerSpread = Math.abs(
							rightThumb.x - rightPinky.x,
						);
						...
						canvasCtx.font = "12px Arial";
						canvasCtx.fillText(
							`R-Spread: ${rightFingerSpread.toFixed(2)}`,
							videoWidth - 170,
							20,
						);
				...
				if (results.faceLandmarks) {
					...
					if (faceLandmarks[mouthStart] && faceLandmarks[mouthEnd]) {
						canvasCtx.beginPath();
						canvasCtx.moveTo(
							faceLandmarks[mouthStart].x * videoWidth,
							faceLandmarks[mouthStart].y * videoHeight,
						);
						canvasCtx.lineTo(
							faceLandmarks[mouthEnd].x * videoWidth,
							faceLandmarks[mouthEnd].y * videoHeight,
						);
						canvasCtx.stroke();
					}
					...
					if (finalExpression === "smile")
						expressionColor = "#00FF00";
					else if (finalExpression === "frown")
						expressionColor = "#FF6600";
					else if (finalExpression === "mad")
						expressionColor = "#FF0000";

					canvasCtx.fillStyle = expressionColor;
					canvasCtx.font = "bold 16px Arial";
					canvasCtx.fillText(
						`😊 Expression: ${finalExpression.toUpperCase()}`,
						10,
						videoHeight - 60,
					);
					...
					if (eyeAnalysis.contact) {
						...
						canvasCtx.fillText(
							`👁️ LOOKING: ${eyeAnalysis.gaze.toUpperCase()}`,
							10,
							videoHeight - 40,
						);
					}
					...
					if (mouthTop && mouthBottom) {
						const mouthOpenness = Math.abs(
							mouthBottom.y - mouthTop.y,
						);
						canvasCtx.fillStyle = "#CCFFCC";
						canvasCtx.font = "12px Arial";
						canvasCtx.fillText(
							`Mouth: ${(mouthOpenness * 100).toFixed(1)}%`,
							10,
							videoHeight - 20,
						);
					}
...
function Session() {
	...
	const { script, memorizeMode: initialMemorizeMode = false } =
		location.state || {};
	...
	const startVolumeTracking = async () => {
		try {
			...
			const pollVolume = () => {
				...
				if (smoothingBuffer.length > SMOOTH_WINDOW)
		...
		recognition.onresult = (event) => {
			...
			newWords.forEach((word, newWordIdx) => {
				...
				if (effectiveFillerWords.includes(lowerWord)) {
					const fillerGlobalIdx =
						lastWordCountRef.current + newWordIdx;
					const contextStart = Math.max(0, fillerGlobalIdx - 3);
					const contextEnd = Math.min(
						wordsArray.length,
						fillerGlobalIdx + 4,
					);
					const contextWords = wordsArray.slice(
						contextStart,
						contextEnd,
					);
					const fillerLocalIdx = fillerGlobalIdx - contextStart;

					contextWords[fillerLocalIdx] =
						`__${contextWords[fillerLocalIdx]}__`;
	...
	const analyzeSpeech = (
		...
		const wpm =
			totalWords > 0 ? Math.round(totalWords / durationMinutes) : 0;
		...
		if (fillerCount === 0) {
			...
			feedback.push(
				"High filler usage detected. Practice intentional pauses.",
			);
		}

		if (wpm < IDEAL_MIN_WPM) {
			feedback.push(
				"Your pace is slightly slow. Increase energy and projection.",
			);
		...
		return {
			summary: {
				durationSeconds,
				durationFormatted: `${Math.floor(durationSeconds / 60)}m ${
					durationSeconds % 60
				}s`,
				totalWords,
				wpm,
			...
			<div className="session-shell">
				<div className="session-header reveal-session">
					...
					<p className="session-subtitle">
						Practice your delivery in real time with pacing,
						transcript, volume, and body tracking.
					</p>
				</div>
					...
					<section className="session-panel controls-panel reveal-session reveal-delay-1">
						...
						<div className="session-action-row">
							...
							<div className="session-tooltip-wrap">
								<button
									type="button"
									className={`session-btn memorize-btn ${
										isMemorizeMode ? "active" : ""
									}`}
								...
								<span className="session-hover-text">
									It hides the script while you speak, then
									shows it again when you become quiet.
								</span>
							</div>
						</div>
					</section>
					...
					<section
						...
						<div className="analysis-grid">
							...
							<div className="metric-card">
								...
								<div
									className={`metric-pill ${getFeedbackClass()}`}>
							...
							<div className="metric-card">
								...
								<div
									className={`volume-display ${volColorClass}`}>
						...
						<div className="transcript-body">
							{transcript ? (
								...
								<p className="transcript-placeholder">
									Your live transcript will appear here once
									the session starts.
								</p>
							)}
						</div>
