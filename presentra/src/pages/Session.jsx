...
const HandTracker = ({ onTrackingUpdate, onCleanup }) => {
	...
	const detectMouthExpression = (faceLandmarks) => {
		...
		// FROWN detection
		if (
			mouthOpenness >= refs.frownMouthOpen.min &&
			mouthOpenness <= refs.frownMouthOpen.max &&
			avgCornerRise < refs.frownCornerDrop
		) {
			console.log("✓ FROWN:", {
				mouthOpenness: mouthOpenness.toFixed(3),
				cornerDrop: avgCornerRise.toFixed(3),
			});
			return "frown";
		}

		console.log("Mouth metrics:", {
			mouthOpenness: mouthOpenness.toFixed(3),
			mouthWidth: mouthWidth.toFixed(3),
			cornerRise: avgCornerRise.toFixed(3),
		});
	...
	const detectHandMovement = (handLandmarks, lastHandPos) => {
		...
		if (
			thumb &&
			pinky &&
			thumb.visibility > 0.3 &&
			pinky.visibility > 0.3
		) {
